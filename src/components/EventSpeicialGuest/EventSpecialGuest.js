...
class EventSpecialGuest extends Component {
  state = {
    guestName: "",
    guestEmail: "",
    guestList: [],
  ...
  componentDidMount() {
    axios
      .get("special-guest/get-guests?event-id=" + this.props.match.params.id)
      .then(response => {
        let guestList = [];

        let dump = response.data.map(user => {
          let struct = {
            specialGuestId: user.specialGuestId,
            userId: user.specialUser.userId,
            name: user.specialUser.name,
            email: user.specialUser.email,
            profilePic: user.specialUser.profilePic,
            ...
            <Comment.Group>
              <Comment>
                ...
                <Comment.Content>
                  ...
                  <Comment.Text>{guest.email}</Comment.Text>
                  <Comment.Actions>
    ...
    return (
      <Grid columns={2} stackable>
        <Modal
          ...
          style={{ width: "55%", height: "90%", clear: "both" }}
          dimmer={dimmer}
          open={open}
          onClose={this.close}
        >
          <Modal.Header>Special Guests</Modal.Header>
          <Modal.Content scrolling>
            <Modal.Description>
              <Header>Event Special Guests.</Header>
              {modelContent}
            </Modal.Description>
          </Modal.Content>
          <Modal.Actions>
          ...
          <Grid columns={2} stackable>
            <Grid.Column width={8}>
              <Segment>
                <Form unstackable onSubmit={this.addGuestsToList}>
                  <Form.Input
                    onChange={this.elementNameHandler}
                    icon="user"
                    iconPosition="left"
                    label="Guest name"
                    placeholder="Name"
                  ...
                  <Form.Input
                    type="email"
                    onChange={this.elementEmailHandler}
                    icon="at"
                    iconPosition="left"
                    label="Guest email"
                    placeholder="Email"
            ...
            <Grid.Column width={8}>
              <Menu vertical fluid>
                ...
                {this.state.guestList.length
                  ? this.state.guestList.map(one => (
                      <React.Fragment>
                        <Menu.Item
                          name={one.name}
                          active={activeItem === one.name}
              ...
              <Segment>
                <Header size="medium">View your current special guests</Header>
                <Button onClick={this.show("blurring")} color="violet" fluid>
                  View Guests
                </Button>
              </Segment>
            </Grid.Column>
...
export default EventSpecialGuest;
